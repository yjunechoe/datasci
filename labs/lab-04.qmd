---
title:  "Lab 4: Distributions and sampling"
...
(a) Write code to generate 200 data points, sampled from a gaussian distribution with a mean of 0 and a standard deveiation of 1.
...
```{r}
#| echo: false
data <- tibble(y = runif(500, max = 10, min = 5 ))
...
(b) Suppose we want to quantify the spread of the sampling distribution. What method could we choose? Choose all that apply. 
...
(c) For a typical experiment, how many samples from the population is practical for us to take? Enter a number.
...
(a) When we generate the bootstrap sampling distribution, we sample our original sample: 



(b) Suppose we want to generate the bootstrap sampling distribution for the mean of set of data with one variable: `reaction_time`. Write code that uses the `infer` package to accomplish this, generating 1000 samples. 

(c) Which of the following would compute the 68% confidence interval and assign it to the value `ci`?

(d) Suppose we store our bootstrap sampling distribution in `bootstrap_distribution` and we want to visualize the confidence interval we just computed.  Which of the following could we add to the following code? 
